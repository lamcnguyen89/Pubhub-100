...
public class ViewBookDetailsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//Get the info on the book from the database using the Book Data Access Object.
		//Then create an object that can then be accessed by the front end:
		BookDAO dao = DAOUtilities.getBookDAO();
		Book book = dao.getBookByISBN(isbn13);
		request.setAttribute("book", book);
		
		//Get the info on the book tags from the database using the Book_Tag Data Access Object:
		//Then create an object that can then be accessed by the front end:
		TagDAO tagdao = DAOUtilities.getTagDAO();
		List<Tag> tag = tagdao.getTagsByBook(isbn13);
